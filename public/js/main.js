...
(function($) {
    var target = '.u-sidebar',
        ...
        $body = $('body'),
        closeAllExceptThis = Boolean(
            $sidebarInvoker.data('is-close-all-except-this')
        ),
        windowWidth = window.innerWidth,
        closedItems = [];
    ...
    function closeEffect(target, closedItems) {
        ...
        if (closedItems.length > 0 && windowWidth >= 768) {
            ...
            $(items).slideUp(400, function() {
                ...
                $(items)
                    .parent()
                    .removeClass('u-sidebar-nav--opened');
            });
        } else {
            $(target + '.toggled').addClass('action mini');
            $body.addClass('side-nav-on-action');
    ...
    function openEffect(target, closedItems) {
        ...
        if (closedItems.length > 0 && windowWidth >= 768) {
            setTimeout(function() {
                $(closedItems.toString())
                    .parent()
                    .addClass('u-sidebar-nav--opened');
                $(
                    'body:not(.sidebar-tablet-closed) ' + closedItems.toString()
                ).slideDown(400);
            }, 300);
        }
    }
    ...
    $(window).on('load', function() {
        ...
        if ($('.u-sidebar-nav--opened').length) {
            $(
                '.u-sidebar-nav--opened > .u-sidebar-nav-menu__link[data-target]'
            ).each(function() {
        ...
        $('.u-sidebar-nav-menu__link[data-target]').on('click', function(e) {
            ...
            if (closeAllExceptThis) {
                closedItems = [];
                $('.u-sidebar-nav-menu--top-level ul:not("' + itemTarget + '")')
                    .slideUp(400)
                    .parent()
                    .removeClass('u-sidebar-nav--opened');
                $(itemTarget)
                    .slideToggle(400)
                    .parent()
                    .toggleClass('u-sidebar-nav--opened');
                if (
                    $(this)
                        .parent()
                        .hasClass('u-sidebar-nav--opened')
                ) {
        ...
        $(window)
            .on('resize', function() {
                ...
                if (windowWidth < 768) {
                    $body
                        .removeClass(
                            'tablet-mode desktop-mode side-nav-on-action sidebar-tablet-closed'
                        )
                        .addClass('mobile-mode sidebar-mobile-closed');
                } else if (windowWidth >= 768 && windowWidth <= 992) {
                    $body
                        .removeClass(
                            'mobile-mode sidebar-mobile-closed desktop-mode side-nav-on-action'
                        )
                        .addClass('tablet-mode sidebar-tablet-closed');
                } else {
                    $body
                        .removeClass(
                            'tablet-mode mobile-mode sidebar-mobile-closed sidebar-tablet-closed has-tablet-opened-items'
                        )
                ...
                if (windowWidth >= 768 && windowWidth <= 992) {
                    ...
                    if (!$body.hasClass('side-nav-on-action')) {
                        ...
                        $body.hasClass('side-nav-on-action desktop-mode')
                    ) {
                        openEffect(target, closedItems);
                    }
                }
